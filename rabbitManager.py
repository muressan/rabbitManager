        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            for line_number, line in enumerate(file, 1):
                ...
                try:
                    ...
                    print(f"Erro ao ler JSONL na linha {line_number}: {exc}")
                    continue
...
def show_help():
    ...
    print("  rabbitManager <nome_da_fila> <lista.jsonl>")
    print("  rabbitManager --exchange <nome_da_exchange> --exchange-type <direct|topic|fanout> [--routing-key <routing_key>] <lista.jsonl>")
    print("")
    print("Exemplos:")
    ...
    print("")
    print("  7. rabbitManager fila lista.jsonl")
    print("     Posta mensagens em uma fila usando um arquivo JSONL com uma mensagem por linha.")
    print("")
    print("  8. rabbitManager --exchange eventos.direct --exchange-type direct --routing-key cliente.rk lista.jsonl")
    print("     Posta mensagens em uma exchange direct.")
    print("")
    print("  9. rabbitManager --exchange eventos.topic --exchange-type topic --routing-key cliente.* lista.jsonl")
    print("     Posta mensagens em uma exchange topic.")
    print("")
    print("  10. rabbitManager --exchange eventos.broadcast --exchange-type fanout lista.jsonl")
...
def parse_arguments():
    ...
    if args.exchange or args.exchange_type or args.routing_key:
        if not args.exchange or not args.exchange_type:
            show_help()
        if len(args.positionals) != 1 or not args.positionals[0].endswith('.jsonl'):
            show_help()
    ...
    if second_arg.endswith('.jsonl') or second_arg == '':
        if args.one_file_per_msg or args.batch_size != DEFAULT_BATCH_SIZE or args.prefetch_count != DEFAULT_PREFETCH_COUNT or args.ack_mode != 'message' or args.no_ack:
            show_help()
        return {
            'queue_name': queue_name,
